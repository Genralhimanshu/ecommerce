import React from 'react'
import { useEffect } from 'react';
import payment from '../assets/payment.png'
import shipping from '../assets/shipping.png'
import refund from '../assets/return.png'
import gift from '../assets/gift.png'
import AOS from 'aos';
import 'aos/dist/aos.css';


const Services = () => {
  useEffect(() => {
    AOS.init({
      offset: 100,
      duration: 2000,
      easing: 'ease-in-out',
    });
    AOS.refresh();
  },[]);
  return (
    <div className='w-full lg:px-20 px-5 pt-[0px] pb-[80px] grid lg:grid-cols-4 grid-cols-1 justify-center items-center gap-10'>
      <div data-aos="zoom-in" data-aos-delay="100" className=' flex flex-col justify-center items-center gap-2'>
        <img src={shipping} alt="shipping" className=' mb-[20px] w-[60px]
        ' />
        <h1 className='text-black text-xl font-semibold'>India Shipping</h1>
        <p className='text-[17px] text-gray-500 text-center'>Free shipping on all orders over Rs.500</p>
        
      </div>
      <div data-aos="zoom-in" data-aos-delay="200" className=' flex flex-col justify-center items-center gap-2'>
        <img src={payment} alt="payment" className=' mb-[20px] w-[60px] ' /> 
        <h1 className='text-black text-xl font-semibold'>Secure Payment</h1>
        <p className='text-[17px] text-gray-500 text-center'>100% secure payment</p>
      </div>
      <div data-aos="zoom-in" data-aos-delay="300" className=' flex flex-col  justify-center items-center gap-2'>
        <img src={refund} alt="refund" className=' mb-[20px] w-[60px] ' />
        <h1 className='text-black text-xl font-semibold'>Easy Return</h1>
        <p className='text-[17px] text-gray-500 text-center'>Simple returns policy</p>
      </div>
      <div data-aos="zoom-in" data-aos-delay="400" className=' flex flex-col
      justify-center items-center gap-2'>
        <img src={gift} alt="gift" className=' mb-[20px] w-[60px] ' />
        <h1 className='text-black text-xl font-semibold'>Gift Wrapping</h1>
        <p className='text-[17px] text-gray-500 text-center'>Gift wrapping available</p>
      
      </div>
      
    </div>
  )
}

export default Services
